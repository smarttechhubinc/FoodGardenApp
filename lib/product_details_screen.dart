...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  final Map<String, Map<String, dynamic>> _hardinessZones = {
    'Zone 1': {'tempRange': 'Below -50°F', 'color': '#2C3E5C', 'emoji': '❄️', 'description': 'Extreme cold'},
    'Zone 2': {'tempRange': '-50°F to -40°F', 'color': '#3E5A8A', 'emoji': '❄️', 'description': 'Very cold'},
    'Zone 3': {'tempRange': '-40°F to -30°F', 'color': '#4F7AB3', 'emoji': '🌡️', 'description': 'Cold winters'},
    'Zone 4': {'tempRange': '-30°F to -20°F', 'color': '#609CD9', 'emoji': '🌡️', 'description': 'Cold climate'},
    'Zone 5': {'tempRange': '-20°F to -10°F', 'color': '#71BDFF', 'emoji': '🌿', 'description': 'Temperate'},
    'Zone 6': {'tempRange': '-10°F to 0°F', 'color': '#8ACC66', 'emoji': '🌿', 'description': 'Mild winters'},
    'Zone 7': {'tempRange': '0°F to 10°F', 'color': '#A5D95E', 'emoji': '☀️', 'description': 'Warm'},
    'Zone 8': {'tempRange': '10°F to 20°F', 'color': '#BFF055', 'emoji': '☀️', 'description': 'Subtropical'},
    'Zone 9': {'tempRange': '20°F to 30°F', 'color': '#D9FF4C', 'emoji': '🌴', 'description': 'Hot'},
    'Zone 10': {'tempRange': '30°F to 40°F', 'color': '#F2F242', 'emoji': '🌴', 'description': 'Tropical'},
  };
  ...
  Future<void> _getAddressFromCoordinates(double lat, double lng) async {
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        setState(() {
          _locationAddress = parts.isNotEmpty ? parts.join(', ') : 'Location available on map';
          print('📍 Address found: $_locationAddress');
        });
      } else {
        setState(() {
      ...
      setState(() {
        _locationAddress = _productData['location_text'] ?? 'Address available on map';
      });
    }
  }

  Future<void> _getAddressFromUserLocation() async {
    if (_userLocation == null) return;
    
    setState(() {
      _isLoadingDeliveryAddress = true;
    });
    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        setState(() {
          _deliveryAddress = parts.isNotEmpty ? parts.join(', ') : 'Your location';
  ...
  Future<void> _getUserLocation() async {
    ...
    try {
      ...
      await _getAddressFromUserLocation();
  ...
  void _showRequestDialog() {
    ...
    String selectedDeliveryOption = _deliveryOption;
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Center(
                  child: Text(
                    'Choose how you want to receive this produce',
                    style: TextStyle(
                      fontSize: 14,
                      color: Colors.grey,
                    ),
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: selectedDeliveryOption == 'pickup'
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            padding: const EdgeInsets.symmetric(vertical: 12),
                            decoration: BoxDecoration(
                              color: selectedDeliveryOption == 'delivery'
                                  ? const Color(0xFF39AC86)
                                  : Colors.transparent,
                              borderRadius: BorderRadius.circular(8),
                            ),
                ...
                // Delivery address field (only shown if delivery is selected)
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            prefixIcon: const Icon(Icons.location_on, color: Color(0xFF39AC86)),
                          ),
                          onChanged: (value) {
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          final deliveryAddress = selectedDeliveryOption == 'delivery'
                              ? (customDeliveryAddress ?? _deliveryAddressController.text)
                              : null;
                          await _sendRequest(
                            requestedQuantity, 
  ...
  Future<void> _sendRequest(int quantity, String message, String deliveryOption, String? deliveryAddress) async {
    setState(() {
      _isRequesting = true;
    });
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        // Add delivery info to request data
        final requestData = {
          'id': request['id'],
          'quantity': quantity,
          'status': 'pending',
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Request sent! Waiting for gardener to respond.'),
          ...
          SingleChildScrollView(
            child: Column(
              children: [
                ...
                // Compact Hardiness Zone Card
                if (_hardinessZone != null && !_isLoadingZone)
                  Container(
                    margin: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                    decoration: BoxDecoration(
                      color: Color(int.parse(_zoneInfo['color'].replaceFirst('#', '0xFF'))).withOpacity(0.1),
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          height: 4,
                          decoration: BoxDecoration(
                            color: Color(int.parse(_zoneInfo['color'].replaceFirst('#', '0xFF'))).withOpacity(0.5),
                            shape: BoxShape.circle,
                          ),
                        ),
                        const SizedBox(width: 6),
                ...
                // Pickup Location Section (simplified)
                Container(
