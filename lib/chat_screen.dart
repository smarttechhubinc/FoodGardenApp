...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadRequestData() async {
    ...
    try {
      ...
      // If not, check API for existing request
  ...
  Future<void> _acceptRequest() async {
    ...
    // Make sure only owner can accept
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final currentUserId = authProvider.userId;
  ...
  Future<void> _declineRequest() async {
    ...
    // Make sure only owner can decline
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final currentUserId = authProvider.userId;
  ...
  Widget build(BuildContext context) {
    ...
    // FIXED: Determine roles from the request data
    bool isOwner = false;
    bool isRequester = false;
    
    if (_currentRequest != null) {
      // If there's a request, check based on request data
      isOwner = _currentRequest!['owner_id'] == currentUserId;
      isRequester = _currentRequest!['requester_id'] == currentUserId;
    } else {
      // Fallback to comparing with recipientId
      isOwner = currentUserId == widget.recipientId;
      isRequester = !isOwner;
    }
    ...
    print('👤 recipientId: ${widget.recipientId}');
    print('👤 isOwner: $isOwner');
    print('👤 isRequester: $isRequester');
    print('📦 _currentRequest: ${_currentRequest?['status']}');
    print('📦 _isLoadingRequest: $_isLoadingRequest');
    
    // Determine if request card should be shown (only for owner with pending request)
